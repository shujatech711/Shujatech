...
const checkUserJoinedChannels = async (userId) => {
  const channels = ['@skcybersecuritytricks', '@shujatechHelpZone'];
  let allJoined = true;
    ...
    `🚨 *You must join our official channels before pairing.*`,
    {
      ...
      reply_markup: {
        inline_keyboard: [
          ...
          [{ text: '📢 Channel 2', url: 'https://t.me/shujaaking' }],
...
bot.onText(/\/start/, async (msg) => {
  ...
  await bot.sendPhoto(
    ...
    {
      caption: `🪀 *𝙏𝙝𝙚 ꨄ ༼〠⸙ՏᎻႮᎫᎪ ͲᎬᏟᎻ ᎷᎠ〠༽ 𝙈𝘿 ꨄ*\n\n╔════════════════════╗\n ⤷ /pair <wa_number>\n ⤷ /unpair <wa_number>\n╚════════════════════╝`,
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [
          [{ text: "👑 Owner", url: "https://t.me/shujaaking" }]
        ]
      }
    }
  );
  ...
  if (!allJoined) {
    return bot.sendMessage(chatId,
      `🚨 *You must join our official channels before pairing.*`,
      {
        ...
        reply_markup: {
          inline_keyboard: [
            [{ text: '📢 Channel 1', url: 'https://t.me/shujaaking' }],
            [{ text: '📢 Channel 2', url: 'https://t.me/shujaaking' }],
            [{ text: '👥 Group', url: 'https://t.me/helpzonegc' }],
            [{ text: '✅ I have joined', callback_data: 'check_join' }]
          ]
